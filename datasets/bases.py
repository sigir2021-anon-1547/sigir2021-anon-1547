...
class ReidBaseDataModule(pl.LightningDataModule):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.num_instances = kwargs.get('num_instances') if 'num_instances' in kwargs else 4
    ...
    def _print_dataset_statistics(self, train, query=None, gallery=None):
        ...
        num_gallery_pids, num_gallery_imgs, num_gallery_cams = self._get_imagedata_info(gallery)
    ...
    def train_dataloader(self, cfg, trainer, sampler_name:str='random_identity', **kwargs):
        ...
        return DataLoader(self.train, self.cfg.SOLVER.IMS_PER_BATCH, num_workers=self.num_workers, shuffle=False, sampler=sampler, 
                        collate_fn=collate_fn_alternative, **kwargs)
                        
    def val_dataloader(self):
        sampler = SequentialSampler(self.val)  ## This get replaced with ddp mode by lightning
        return DataLoader(self.val, self.cfg.TEST.IMS_PER_BATCH, num_workers=self.num_workers, sampler=sampler, shuffle=False)

    def test_dataloader(self):
        sampler = SequentialSampler(self.train)  ## This get replaced with ddp mode by lightning
        return DataLoader(self.train, self.cfg.TEST.IMS_PER_BATCH, num_workers=self.num_workers, sampler=sampler, shuffle=False)
...
class COCODatasetBase(ReidBaseDataModule):
    ...
    def setup(self):
        ...
        self.query_json = self._load_json(self.json_query_path)  ## Just replace train with proper set name. JSONs filenames should  be consistent
        self.gallery_json = self._load_json(self.json_gallery_path)  ## Just replace train with proper set name. JSONs filenames should  be consistent
        ...
        self.train = BaseDatasetLabelledPerPid(train_dict, transforms_base.build_transforms(is_train=True), self.num_instances, self.DATALOADER.USE_RESAMPLING) ## Only train needs to be alternative dataset and sampler
    ...
    def _process_dir(self, images_path, json_file, relabel=False):
        ...
        if relabel: pid2label = {pid: label for label, pid in enumerate(unique_pair_ids_set)}
        ...
        for idx, pair_id in enumerate(tqdm(unique_pair_ids_set)):
            ...
            if relabel: pair_id = pid2label[pair_id]
...
class BaseDatasetLabelledPerPid(Dataset):
    ...
    def __getitem__(self, pid):
        ...
        """
        pid = int(pid)
        list_of_samples = self.samples[pid][:]   # path, target, camid, idx <- in each inner tuple
        _len = len(list_of_samples)
        assert _len > 1, f'len of samples for pid: {pid} is <=1. len: {len_}, samples: {list_of_samples}'
        ...
        for _ in range(choice_size):
            ...
            out.append((img, target, camid, idx, True))  ## True stand if the sample is real or mock

        if needPad:
            num_missing = self.num_instances - _len
            assert num_missing != self.num_instances, f"Number of missings sample in the batch is equal to num_instances. PID: {pid}"
            if self.resample:
                assert len(list_of_samples) > 0
                resampled = np.random.choice(range(len(list_of_samples)), size=num_missing, replace=True)
        ...
        assert len(out) == self.num_instances, f'Number of returned tuples per id needs to be equal self.num_instance. It is: {len(out)}'
