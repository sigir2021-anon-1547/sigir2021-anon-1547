...
class CTLModel(ModelBase):
    ...
    def __init__(self, cfg=None, **kwargs):
        ...
        self.losses_names = ['query_xent', 'query_triplet', 'query_center', 'centroid_triplet']
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=None):
        ...
        if self.hparams.SOLVER.USE_WARMUP_LR:
            if self.trainer.current_epoch < self.hparams.SOLVER.WARMUP_EPOCHS:
                lr_scale = min(1., float(self.trainer.current_epoch + 1) / float(self.hparams.SOLVER.WARMUP_EPOCHS))
        ...
        # query
        contrastive_loss_query, _, _ = self.contrastive_loss(features, class_labels, mask=isReal)
        contrastive_loss_query = contrastive_loss_query * self.hparams.SOLVER.QUERY_CONTRASTIVE_WEIGHT

        class_labels_real = class_labels[isReal]
        features_real = features[isReal]
        center_loss = self.hparams.SOLVER.CENTER_LOSS_WEIGHT * self.center_loss(features_real, class_labels_real)
        ...
        centroids_mask = rearrange(masks, 'i (ins s) -> i ins s', s=self.hparams.DATALOADER.NUM_INSTANCE)
        padded_tmp = rearrange(padded, 'i (ins s) dim -> i ins s dim', s=self.hparams.DATALOADER.NUM_INSTANCE) 
        valid_inst = centroids_mask.sum(-1)
        valid_inst_bool = centroids_mask.sum(-1).bool()
        centroids_emb = padded_tmp.sum(-2) / valid_inst.masked_fill(valid_inst==0, 1).unsqueeze(-1)
        ...
        for i in range(self.hparams.DATALOADER.NUM_INSTANCE):
            ...
            current_centroids = current_centroids[torch.abs(current_centroids).sum(1) > 1e-7]
            embeddings_concat = torch.cat((query_feat, current_centroids))
            labels_concat = torch.cat((current_labels, current_labels))

            contrastive_loss, dist_ap, dist_an = self.contrastive_loss(embeddings_concat, labels_concat)
        ...
        contrastive_loss_step = torch.mean(torch.stack(contrastive_loss_total)) * self.hparams.SOLVER.CENTROID_CONTRASTIVE_WEIGHT
        ...
        total_loss = contrastive_loss_step + center_loss + xent_query + contrastive_loss_query

        self.manual_backward(total_loss, optimizer=opt)
        opt.step()

        for param in self.center_loss.parameters():
            param.grad.data *= (1. / self.hparams.SOLVER.CENTER_LOSS_WEIGHT)
        opt_center.step()
        ...
        log_data = {
            "step_dist_ap": float(dist_ap), "step_dist_an": float(dist_an),
