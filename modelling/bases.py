...
class ModelBase(pl.LightningModule):
    ...
    def __init__(self, cfg=None, test_dataloader=None, **kwargs):
        ...
        self.contrastive_loss = TripletLoss(self.hparams.SOLVER.MARGIN, self.hparams.SOLVER.DISTANCE_FUNC)

        d_model = self.hparams.MODEL.BACKBONE_EMB_SIZE
        self.xent = CrossEntropyLabelSmooth(num_classes=self.hparams.num_classes)
        self.center_loss = CenterLoss(num_classes=self.hparams.num_classes, feat_dim=d_model)
    ...
    def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_idx, 
                        ...
                        using_lbfgs=False, **kwargs):
                        
        if self.hparams.SOLVER.USE_WARMUP_LR:
            if epoch < self.hparams.SOLVER.WARMUP_EPOCHS:
                lr_scale = min(1., float(epoch + 1) / float(self.hparams.SOLVER.WARMUP_EPOCHS))
                for pg in optimizer.param_groups:
                    pg['lr'] = lr_scale * self.hparams.SOLVER.BASE_LR

        super().optimizer_step(epoch=epoch, optimizer=optimizer, optimizer_idx=optimizer_idx, batch_idx=batch_idx, 
                        optimizer_closure=optimizer_closure, on_tpu=on_tpu, using_native_amp=using_native_amp, using_lbfgs=using_lbfgs, **kwargs)

    def training_step(self, batch, batch_idx, opt_idx=None):
        raise NotImplementedError('A used model should have its own training_step method implemented')
    ...
    @rank_zero_only
    def validation_create_centroids(self, embeddings, labels, camids):
        ...
        centroids_embeddings = torch.cat((embeddings_query, centroids_embeddings), dim=0)
    ...
    def get_val_metrics(self, embeddings, labels, camids):
        self.r1_map_func = R1_mAP(pl_module=self, num_query=self.hparams.num_query, feat_norm=self.hparams.TEST.FEAT_NORM)
        cmc, mAP, all_topk = self.r1_map_func.compute(feats=embeddings.float(), pids=labels, camids=camids)
    ...
    def validation_epoch_end(self, outputs):
        if self.trainer.global_rank == 0 and self.trainer.local_rank == 0:
            ...
            labels = torch.cat([x.pop("labels") for x in outputs]).detach().cpu().numpy()
            camids = torch.cat([x.pop("camid") for x in outputs]).cpu().detach().numpy()
            del outputs
            if self.hparams.MODEL.USE_CENTROIDS:
                embeddings, labels, camids = self.validation_create_centroids(embeddings, labels, camids)
            if self.trainer.global_rank == 0 and self.trainer.local_rank == 0:
    ...
    def eval_on_train(self):
        if self.trainer.global_rank == 0 and self.trainer.local_rank == 0:
            ...
            cmc, mAP, all_topk = self.r1_map_func.compute(feats=embeddings, pids=labels, camids=camids)
    ...
    def create_masks_train(class_labels):
        ...
        max_gal_num = max([len(item) for item in labels_dict.values()])  ## TODO Should allow usage of all permuations
