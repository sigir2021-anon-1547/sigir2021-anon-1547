...
try:
    from utils.eval_cython.eval_reid_cy import evaluate_cy
    IS_CYTHON_AVAI = True
except ImportError:
    ...
    warnings.warn(
        ...
        'To enable Cython evaluation got to utils/eval_cython/ and run\n'
...
class R1_mAP:
    ...
    def compute(self, feats, pids, camids):
        if self.feat_norm:
        ...
        if IS_CYTHON_AVAI:
            cmc, mAP, all_topk, single_performance = evaluate_cy(indices, q_pids, g_pids, q_camids, g_camids, 50)
        else:
            cmc, mAP, all_topk, single_performance = eval_func(indices, q_pids, g_pids, q_camids, g_camids, 50)
