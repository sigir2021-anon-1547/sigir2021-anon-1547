...
def run_single(cfg, method, logger_save_dir):
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=os.path.join(logger.log_dir, 'checkpoints'), filename='{epoch}',
        monitor=cfg.SOLVER.MONITOR_METRIC_NAME, mode=cfg.SOLVER.MONITOR_METRIC_MODE, verbose=True
    )
    ...
    dm = init_dataset(cfg.DATASETS.NAMES, cfg=cfg, num_workers=cfg.DATALOADER.NUM_WORKERS)
    dm.setup()
    test_dataloader=None

    trainer = pl.Trainer(gpus=cfg.GPU_IDS, max_epochs=cfg.SOLVER.MAX_EPOCHS, logger=loggers, fast_dev_run=False,
                        check_val_every_n_epoch=cfg.SOLVER.EVAL_PERIOD, 
                        accelerator=cfg.SOLVER.DIST_BACKEND,
                        num_sanity_val_steps=0,
                        replace_sampler_ddp=False, checkpoint_callback=checkpoint_callback,
    ...
    train_loader = dm.train_dataloader(cfg, trainer, sampler_name=cfg.DATALOADER.SAMPLER,
                                            drop_last=cfg.DATALOADER.DROP_LAST)
    val_dataloader = dm.val_dataloader()
    if cfg.TEST.ONLY_TEST:
        method = method.load_from_checkpoint(cfg.MODEL.PRETRAIN_PATH, cfg=cfg, num_query=dm.num_query, num_classes=dm.num_classes, use_multiple_loggers=True if len(loggers) > 1 else False)
        ...
        if cfg.MODEL.RESUME_TRAINING:
            method = method.load_from_checkpoint(cfg.MODEL.PRETRAIN_PATH, num_query=dm.num_query, num_classes=dm.num_classes, use_multiple_loggers=True if len(loggers) > 1 else False)
        else:
            method = method(cfg, test_dataloader=test_dataloader, num_query=dm.num_query, num_classes=dm.num_classes, use_multiple_loggers=True if len(loggers) > 1 else False)
        trainer.fit(method, train_dataloader=train_loader, val_dataloaders=[val_dataloader])
        method.hparams.MODEL.USE_CENTROIDS = not method.hparams.MODEL.USE_CENTROIDS
        trainer.test(model=method, test_dataloaders=val_dataloader)
        method.hparams.MODEL.USE_CENTROIDS = not method.hparams.MODEL.USE_CENTROIDS
...
def run_main(cfg, method, logger_save_dir):
    ...
    cfg.LOG_DIR = f'logs/{cfg.DATASETS.NAMES}' if cfg.OUTPUT_DIR=="" else cfg.OUTPUT_DIR

    if cfg.REPRODUCIBLE:
        for seed in range(cfg.REPRODUCIBLE_SEED, cfg.REPRODUCIBLE_SEED + cfg.REPRODUCIBLE_NUM_RUNS):
